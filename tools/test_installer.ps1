...
$setupFile = Join-Path $repoRoot 'dist/installer/Share4AI-Setup-1.1.0-windows-x64.exe'
$report = Join-Path $repoRoot 'build/installed-smoke.json'
try {
    foreach ($attempt in 1..2) {
        $setupProcess = Start-Process -FilePath $setupFile -ArgumentList @('/VERYSILENT','/SUPPRESSMSGBOXES','/NORESTART',('/DIR="' + $testDir + '"')) -WindowStyle Hidden -Wait -PassThru
        if ($setupProcess.ExitCode -ne 0) { throw 'Installation failed' }
        if (-not (Test-Path -LiteralPath $desktopLink) -or -not (Test-Path -LiteralPath $startLink)) { throw 'Shortcuts missing' }
        if (-not (Test-Path -LiteralPath (Join-Path $testDir '_internal/python312.dll'))) { throw 'Bundled Python missing' }
        # The tested process has no Python or Tcl environment and no Python on PATH.
